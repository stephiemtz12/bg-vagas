pipeline {
  ...
  environment { 
    AWSCLI = "aws autoscaling describe-auto-scaling-groups --query 'AutoScalingGroups[?contains(Tags[?Key==`Aplicacao`].Value, `API-CANDIDATOS`)].[DesiredCapacity]' --output text"
    VERSAO = '0.2'
  ...
  stages {
    ...
    stage('Checkout Bitbucket'){
      ...
      steps{
        ...
        extensions: [], submoduleCfg: [], userRemoteConfigs: [[credentialsId: 'Jailson-Bitbucket', 
        url: 'https://jailsonsilva@bitbucket.org/vagas/api_recomendacao_candidatos.git']]])
      }
    }
